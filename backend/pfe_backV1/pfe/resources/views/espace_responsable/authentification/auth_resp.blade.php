...
<body class="font-[Poppins] flex items-center justify-center min-h-screen" style="background-color: var(--background-color)">
  <div class="w-full max-w-md p-8 rounded-xl glass-card shadow-xl">
    ...
    <form action="traitement_technicien.php" method="POST" class="space-y-4">
      <div>
        <label class="block mb-1 font-medium text-[var(--text-color)]">Nom d'utilisateur</label>
        <input type="text" name="username" required class="w-full px-4 py-2 border rounded-lg border-[var(--input-border-color)] focus:outline-none focus:ring-2 focus:ring-[var(--secondary-color)]" />
      </div>
      <div>
        <label class="block mb-1 font-medium text-[var(--text-color)]">Mot de passe</label>
        <input type="password" name="password" required class="w-full px-4 py-2 border rounded-lg border-[var(--input-border-color)] focus:outline-none focus:ring-2 focus:ring-[var(--secondary-color)]" />
      </div>
